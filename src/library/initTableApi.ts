...
export function initTableApi(
	...
	const {
		baseUrl = '/', vDao, locale = {},
		analysisParams = (params: any) => params
	} = config || {}
	...
	return [
		{
			...
			callback: (params: Record<string, any>): any => {
				...
				if (analysisParams) {
					return analysisParams(vDao[tableName])
				}
...

		{
			...
			callback: (params: Record<string, any>): any => {
				...
				if (analysisParams) { return analysisParams(result) }
				return result;
			}
		},
		{
			...
			callback: (params: Record<string, any>): any => {
				...
				if (params.id) {
					...
					if (analysisParams) { return analysisParams({}) }
					return {};
				}
			},
		},
